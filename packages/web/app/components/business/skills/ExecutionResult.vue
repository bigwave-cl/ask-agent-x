...
/** 是否存在未接入成功的平台或自定义目录。 */
const hasLinkFailures = computed(() => [
  ...props.receipt.platformResults,
  ...(props.receipt.customLinkResults ?? []),
...
<template>
  <section class="overflow-hidden rounded-[28px] border bg-card shadow-sm">
    <header class="relative overflow-hidden border-b p-6 sm:p-9">
      ...
      <div class="relative max-w-2xl"><span class="grid size-12 place-items-center rounded-2xl bg-primary text-primary-foreground shadow-lg"><Icon :name="hasLinkFailures ? 'askx-status:warning' : 'askx-status:check'" class="size-5" /></span><h2 class="mt-6 text-2xl font-semibold tracking-[-0.03em] sm:text-3xl">{{ t(mode === 'sync' ? 'skills.syncResultTitle' : hasLinkFailures ? 'skills.resultPartialTitle' : 'skills.resultTitle') }}</h2><p class="mt-2 text-sm leading-6 text-muted-foreground">{{ t(mode === 'sync' ? 'skills.syncResultDescription' : hasLinkFailures ? 'skills.resultPartialDescription' : 'skills.resultDescription') }}</p></div>
    </header>
    <div class="grid gap-3 p-4 sm:p-6">
      ...
      <article v-for="result in receipt.customLinkResults ?? []" :key="result.id" class="flex flex-wrap items-center gap-3 rounded-2xl border border-primary/20 bg-primary/5 p-4">
        <span class="grid size-9 shrink-0 place-items-center rounded-xl bg-primary text-primary-foreground"><Icon name="askx-objects:folder" class="size-4" /></span>
        <div class="min-w-0 flex-1"><strong class="block text-sm">{{ result.name }}</strong><span class="mt-1 block truncate font-mono text-[9px] text-muted-foreground" :title="result.path">{{ result.path }} → {{ result.target }}</span></div>
        <Badge :variant="result.status === 'applied' ? 'secondary' : 'destructive'">{{ t(`skills.${statusKeys[result.status]}`) }}</Badge>
        <ul v-if="result.warnings.length" class="basis-full text-xs text-destructive"><li v-for="warning in result.warnings" :key="warning">{{ warning }}</li></ul>
      </article>
