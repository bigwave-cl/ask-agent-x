...
describe('代码高亮语言与容量规则', () => {
  it('使用显式语言并通过文件扩展名兜底', () => {
    ...
    expect(resolveCodeHighlightLanguage('py')).toBe('python')
    expect(resolveCodeHighlightLanguage('', 'worker.py')).toBe('python')
    expect(resolveCodeHighlightLanguage('', 'main.cpp')).toBe('cpp')
    expect(resolveCodeHighlightLanguage('', 'Dockerfile')).toBe('dockerfile')
    expect(resolveCodeHighlightLanguage('toml')).toBe('ini')
