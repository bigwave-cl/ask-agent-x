...
/** macOS 原生目录选择器使用的 JXA 脚本。 */
const macOSDirectoryPickerScript = `
const app = Application.currentApplication()
app.includeStandardAdditions = true
const selected = app.chooseFolder({
  withPrompt: '选择需要扫描的 Skill 文件夹',
 ...
 */
export async function selectLocalDirectories(): Promise<SelectedLocalDirectory[]> {
  if (process.platform !== 'darwin') throw new Error('当前版本仅支持在 macOS 上使用原生目录选择器。')
  try {
    const { stdout } = await execFileAsync('osascript', ['-l', 'JavaScript', '-e', macOSDirectoryPickerScript], {
      encoding: 'utf8',
      maxBuffer: 1024 * 1024,
    })
